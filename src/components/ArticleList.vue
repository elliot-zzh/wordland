<template>
  ...
  <div class="slide-enter-content">
    <a
      class="list text-very-dark-gray decoration-none p-10px pl-20px mb-20px items-center flex flex-wrap hover:shadow-xl transition-240"
      v-for="article in articlesToShow"
      :class="mode === 'Full' ? 'rounded-10px' : 'rounded-8px'"
      :href="article.url"
    >
      <span
        class="pushable article-link text-sea-blue text-1.35em font-bold m-5px ml-2px mr-0 flex items-center"
      >
      ...
      <span class="i-tabler-ballpen ml-3px transition-160"></span>
      <div class="w-100%"></div>
    ...
    >
      <span>全部文章</span>
      <span class="ml-1px i-tabler-chevrons-right transition-480"></span>
    </a>
  </div>
</template>
...
.list:hover .i-tabler-chevrons-right {
  opacity: 1;
